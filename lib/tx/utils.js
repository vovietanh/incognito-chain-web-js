...
const prepareInputForReplaceTxNormal = async (inCoinSerialNumbers, hasPrivacy, tokenID, account, rpcClient) => {
  // get all output coins
    ...
    try {
      allOutputCoinStrs = await account.getOutputCoins(tokenID, rpcClient);
    } catch (e) {
...
const prepareInputForTx = async (amountTransfer, fee, hasPrivacy, tokenID, account, rpcClient) => {
  if (DEBUGGING_MODE) {
    console.log("Prepare Input For Tx (Amount, Fee): ", amountTransfer, fee);
  }
  ...
  if (DEBUGGING_MODE) {
    console.log("--List unspentCoin - Len =  ", unspentCoinStrs.length);
    for (let i = 0; i < unspentCoinStrs.length; i++) {
      console.log("---- ", unspentCoinStrs[i])
    }
  }
  ...
  if (DEBUGGING_MODE) {
    console.log("--List unspentCoinExceptSpeningCoin - Len = ", unspentCoinExceptSpendingCoin.length);
    for (let i = 0; i < unspentCoinExceptSpendingCoin.length; i++) {
      console.log("---- ", unspentCoinExceptSpendingCoin[i])
    }
  }

  // total amount transfer and fee
  let totalAmountTransfer = amountTransfer.add(fee);
  console.log("--Total Amount Transfer: ", totalAmountTransfer);

  let respChooseBestCoin;
  try {
    respChooseBestCoin = chooseBestCoinToSpent(unspentCoinExceptSpendingCoin, totalAmountTransfer);
  } catch (e) {
    if (DEBUGGING_MODE) {
  ...
  if (DEBUGGING_MODE) {
    console.log("--List inputCoinsToSpent - Len = ", inputCoinsToSpent.length);
    for (let i = 0; i < inputCoinsToSpent.length; i++) {
      console.log("---- ", inputCoinsToSpent[i])
    }
  }

  if (inputCoinsToSpent.length == 0 && totalAmountTransfer.cmp(new bn(0)) != 0) {
