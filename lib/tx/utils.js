...
const prepareInputForTx = async (amountTransfer, fee, hasPrivacy, tokenID, account, rpcClient) => {
  ...
  if (DEBUGGING_MODE) {
    console.log("--List unspentCoin - Len =  ", length(unspentCoinStrs));
    for (let i = 0; i < length(unspentCoinStrs); i++) {
  ...
  if (DEBUGGING_MODE) {
    console.log("--List unspentCoinExceptSpeningCoin - Len = ", length(unspentCoinExceptSpendingCoin));
    for (let i = 0; i < length(unspentCoinExceptSpendingCoin); i++) {
